...
public class update implements Runnable {// interface with sensors
	...
	public void run() {
			...
			if (serial.isOpen() || !useReal) {
				try {
					if (ready && (System.currentTimeMillis() >= start + delayTime)) {
						...
						mod++;
						if (mod % 20 == 0 && basic.debug_lvl > 9)
							System.out.println("Sending packet: " + mod + " as: " + output);
						if (useReal) {
							serial.write(output);
						}
						if (logTraffic) {
						...
						if (!IS_PI) {// reads fake input data from
		...
		try {
			...
			System.out.println("Error closing serial: " + e);
		}
	 ...
	 * 
	 * @param PI
	 *            If the computer running program is an actual Pi or not
	 */
	public static void setUp(boolean PI) {
		...
		if (IS_PI && useReal) {// only adds even listener if a true pi
			System.out.println("Setting up real coms");
			try {
				serial.addListener(event -> {// add event listener to get data
												// from port
					String payload = "";
					try {
	...
	private static void parseIn(String me) {
		if (basic.logger_lvl > 10)
			debug.log("String recieved from serial @: " + System.currentTimeMillis() + " : " + me);
		...
		if (!me.equals(input)) {
			...
			if (input.length() >= 10 && me.startsWith("Running self test")) {// checks
				...
				if (meSplit.length == 5) {// parses pitch, roll, direction,
	...
	public static void set_motors(int[] motor_vals) {
		...
		for (int i = 0; i < 6; i++) {// converts all motor values to int ascii,
										// separated by commas
	...
	/**
	 * Stops port, shutting down connection from PI to Arduino Stops motors
	 * first
	 */
	public static void stop() {
		...
		if (!serial.isOpen()) {
			...
			try {
				set("[n" + movable.base_speed + "," + movable.base_speed + "," + movable.base_speed + ","
						+ movable.base_speed + "," + movable.base_speed + "," + movable.base_speed + ",");
				// set("[n1500,1500,1500,1500,1500,1500,");//turns off all
				// motors
				Thread.sleep(120);
	...
	public static void logIn(String me) {// TODO make one writer and close it at
											// the end
	...
	public static void resetPort() {
		try {
			...
			System.out.println("Error in update.resetPort(): " + e);
		}
	}
